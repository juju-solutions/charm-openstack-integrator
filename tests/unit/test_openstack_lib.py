...
nrpe_helpers = openstack.nrpe_helpers
...
def test_create_nrpe_check_cmd(config, check, exp_cmd):
    openstack.NAGIOS_PLUGINS_DIR = "/tmp"
    openstack.config = config
    assert openstack.create_nrpe_check_cmd(check) == exp_cmd
...
def test_validate_nrpe_configuration(config, exp_result):
    openstack.config = config
    assert openstack.validate_nrpe_configuration() == exp_result
...
def test_remove_nrpe_check():
    nrpe_setup = MagicMock()
    check = Mock()
    check.name = "test"

    openstack.remove_nrpe_check(nrpe_setup, "test-cmd", check)
    nrpe_setup.remove_check.assert_called_once_with(
        shortname="test", description="", check_cmd="test-cmd")
    nrpe_setup.remove_check.reset_mock()

    # test check nto found
    nrpe_setup.remove_check.side_effect = FileNotFoundError
